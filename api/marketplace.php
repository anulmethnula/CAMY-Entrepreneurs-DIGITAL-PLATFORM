...
function market_ensure_order_schema(PDO $pdo): void {
    ...
    foreach(['supply_mode'=>"VARCHAR(30) NOT NULL DEFAULT 'cash_purchase'",'credit_return_due_at'=>'DATETIME NULL','credit_return_status'=>'VARCHAR(40) NULL'] as $field=>$definition)if(!$pdo->query("SHOW COLUMNS FROM stock_supply_requests LIKE '$field'")->fetch())$pdo->exec("ALTER TABLE stock_supply_requests ADD COLUMN $field $definition");
    $ready=true;
}
function market_credit_return_refresh(array &$state): bool {
    $changed=false;$now=time();
    foreach($state['requests'] ?? [] as &$request)if(($request['supplyMode'] ?? '')==='credit_stock'&&!empty($request['creditReturnDueAt'])&&($request['creditReturnStatus'] ?? '')==='Open'&&strtotime($request['creditReturnDueAt'])<$now){$request['creditReturnStatus']='Overdue — entrepreneur must settle outstanding stock';$request['creditOverdueAt']=date(DATE_ATOM);$changed=true;}
    unset($request);return $changed;
}
function market_state(PDO $pdo, bool $lock = false): array {
    ...
    if(market_credit_return_refresh($state))$changed=true;
...
function market_route(PDO $pdo, string $path, string $method): void {
    ...
    if ($path === '/marketplace/requests' && $method === 'POST') {
        ...
        $supplyMode=(string)($data['supplyMode'] ?? 'cash_purchase');if(!in_array($supplyMode,['cash_purchase','credit_stock'],true))response(['message'=>'Choose a valid stock supply method.'],422);
        if($supplyMode==='credit_stock'){ $person=null;foreach($state['entrepreneurs'] as $entry)if((string)$entry['id']===(string)$user['member_id']){$person=$entry;break;} if(!$person||(float)($person['credit']??0)<=0||(float)($person['used']??0)+$total>(float)$person['credit']){$pdo->rollBack();response(['message'=>'This credit-stock request exceeds your available CAMY credit or you are not credit eligible.'],409);} }
        $request=['id'=>'SUP-'.bin2hex(random_bytes(5)),'entrepreneurId'=>(string)$user['member_id'],'entrepreneurName'=>(string)$user['full_name'],'items'=>$clean,'total'=>$total,'supplyMode'=>$supplyMode,'reference'=>$reference,'receipt'=>'','receiptName'=>basename((string)($data['receiptName'] ?? 'receipt')),'status'=>'Pending','createdAt'=>date(DATE_ATOM)];
        $file='';
        $pdo->prepare('INSERT INTO stock_supply_requests(id,entrepreneur_member_id,payment_reference,receipt_path,total,supply_mode) VALUES(?,?,?,?,?,?)')->execute([$request['id'],$request['entrepreneurId'],$reference,$file,$total,$supplyMode]);foreach($clean as $item){$code='';foreach($state['products'] as $product)if((string)$product['id']===(string)$item['productId']){$code=(string)($product['code'] ?? $product['id']);break;}$pdo->prepare('INSERT INTO stock_supply_request_items(request_id,product_code,quantity,purchase_price) VALUES(?,?,?,?)')->execute([$request['id'],$code,$item['qty'],$item['price']]);}
    ...
    if (preg_match('#^/marketplace/requests/([^/]+)/credit-return-period$#',$path,$matches) && $method==='POST') {
        require_admin($pdo);$data=input();$days=filter_var($data['days'] ?? null,FILTER_VALIDATE_INT);if($days===false||$days<1||$days>365)response(['message'=>'Set a return period between 1 and 365 days.'],422);
        $pdo->beginTransaction();$state=market_state($pdo,true);$index=null;foreach($state['requests'] as $key=>$request)if($request['id']===$matches[1]){$index=$key;break;}if($index===null)response(['message'=>'Credit issue not found.'],404);
        if(($state['requests'][$index]['supplyMode'] ?? '')!=='credit_stock')response(['message'=>'Only credit-stock issues have a return period.'],409);
        $state['requests'][$index]['creditReturnDueAt']=date(DATE_ATOM,time()+$days*86400);$state['requests'][$index]['creditReturnStatus']='Open';$state['requests'][$index]['creditReturnPeriodDays']=$days;$state['requests'][$index]['creditReturnPeriodUpdatedAt']=date(DATE_ATOM);market_save($pdo,$state);$pdo->commit();response(['state'=>$state]);
    }
    if (preg_match('#^/marketplace/requests/([^/]+)/issue-credit$#',$path,$matches) && $method==='POST') {
        require_admin($pdo);$data=input();$days=filter_var($data['returnDays'] ?? 30,FILTER_VALIDATE_INT);if($days===false||$days<1||$days>365)response(['message'=>'Set a return period between 1 and 365 days.'],422);
        $pdo->beginTransaction();$state=market_state($pdo,true);$index=null;foreach($state['requests'] as $key=>$request)if($request['id']===$matches[1]){$index=$key;break;}if($index===null)response(['message'=>'Stock request not found.'],404);$request=$state['requests'][$index];
        if(($request['supplyMode'] ?? '')!=='credit_stock'||$request['status']!=='Pending')response(['message'=>'Only pending credit-stock requests can be issued.'],409);
        $personIndex=null;foreach($state['entrepreneurs'] as $key=>$person)if((string)$person['id']===(string)$request['entrepreneurId']){$personIndex=$key;break;}if($personIndex===null)response(['message'=>'Entrepreneur not found.'],404);
        if((float)($state['entrepreneurs'][$personIndex]['used'] ?? 0)+(float)$request['total']>(float)($state['entrepreneurs'][$personIndex]['credit'] ?? 0))response(['message'=>'This issue exceeds the entrepreneur’s available credit.'],409);
        workflow_reserve($state,$request['items'],null);
        foreach($request['items'] as $item){$found=false;foreach($state['inventory'] as &$inventory)if((string)$inventory['entrepreneurId']===(string)$request['entrepreneurId']&&(string)$inventory['productId']===(string)$item['productId']){$inventory['qty']+=(int)$item['qty'];$found=true;break;}unset($inventory);if(!$found)$state['inventory'][]=['entrepreneurId'=>$request['entrepreneurId'],'productId'=>$item['productId'],'qty'=>(int)$item['qty'],'price'=>$item['price'],'visible'=>true];}
        $state['entrepreneurs'][$personIndex]['used']=round((float)($state['entrepreneurs'][$personIndex]['used'] ?? 0)+(float)$request['total'],2);
        $state['requests'][$index]['status']='Credit issued';$state['requests'][$index]['reserved']=true;$state['requests'][$index]['creditIssuedAt']=date(DATE_ATOM);$state['requests'][$index]['creditReturnPeriodDays']=$days;$state['requests'][$index]['creditReturnDueAt']=date(DATE_ATOM,time()+$days*86400);$state['requests'][$index]['creditReturnStatus']='Open';$state['requests'][$index]['creditOutstanding']=(float)$request['total'];$state['requests'][$index]['reviewedAt']=date(DATE_ATOM);market_save($pdo,$state);$pdo->commit();response(['state'=>$state]);
    }
    if (preg_match('#^/marketplace/requests/([^/]+)/credit-return$#',$path,$matches) && $method==='POST') {
        require_admin($pdo);$data=input();$items=workflow_items($data['items'] ?? []);$condition=(string)($data['condition'] ?? 'returned');if(!in_array($condition,['returned','damaged','lost','not_returned'],true)||!$items)response(['message'=>'Provide returned quantities and a valid condition.'],422);$note=trim((string)($data['note'] ?? ''));if($condition!=='returned'&&strlen($note)<3)response(['message'=>'Record a short damage, loss, or non-return note.'],422);
        $pdo->beginTransaction();$state=market_state($pdo,true);$index=null;foreach($state['requests'] as $key=>$request)if($request['id']===$matches[1]){$index=$key;break;}if($index===null)response(['message'=>'Credit issue not found.'],404);$request=&$state['requests'][$index];if(($request['supplyMode'] ?? '')!=='credit_stock'||!in_array($request['status'],['Credit issued','Credit return review'],true))response(['message'=>'This credit issue is not open for return review.'],409);
        $value=0;foreach($items as $item){$original=null;foreach($request['items'] as $entry)if((string)$entry['productId']===(string)$item['productId']){$original=$entry;break;}$already=(int)($request['creditReturnedQty'][(string)$item['productId']] ?? 0);if(!$original||(int)$item['qty']<1||(int)$item['qty']>$original['qty']-$already)response(['message'=>'A return quantity is invalid.'],422);$request['creditReturnedQty'][(string)$item['productId']]=$already+(int)$item['qty'];$value+=(float)$original['price']*(int)$item['qty'];if($condition==='returned'){foreach($state['products'] as &$product)if((string)$product['id']===(string)$item['productId']){$product['stock']+=(int)$item['qty'];break;}unset($product);foreach($state['inventory'] as &$inventory)if((string)$inventory['entrepreneurId']===(string)$request['entrepreneurId']&&(string)$inventory['productId']===(string)$item['productId']){$inventory['qty']=max(0,(int)$inventory['qty']-(int)$item['qty']);break;}unset($inventory);}}
        if($condition==='returned'){foreach($state['entrepreneurs'] as &$person)if((string)$person['id']===(string)$request['entrepreneurId']){$person['used']=max(0,round((float)$person['used']-$value,2));break;}unset($person);$request['creditOutstanding']=max(0,round((float)($request['creditOutstanding'] ?? $request['total'])-$value,2));}
        $request['creditReturnLog'][]=['at'=>date(DATE_ATOM),'condition'=>$condition,'items'=>$items,'value'=>round($value,2),'note'=>$note];$allReturned=true;foreach($request['items'] as $entry)if((int)($request['creditReturnedQty'][(string)$entry['productId']] ?? 0)<(int)$entry['qty']){$allReturned=false;break;}$request['creditReturnStatus']=$allReturned?'Closed':($condition==='returned'?'Partially returned':'Entrepreneur liable — payment required');$request['status']=$allReturned?'Credit closed':'Credit issued';unset($request);market_save($pdo,$state);$pdo->commit();response(['state'=>$state]);
    }
