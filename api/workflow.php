...
function workflow_route(PDO $pdo,string $path,string $method): void {
    ...
    if(preg_match('#^/marketplace/orders/([^/]+)/payout$#',$path,$payoutMatch)&&$method==='POST'){
        ...
        foreach(['bank','branch','holder','account'] as $bankKey)if(empty($bank[$bankKey])){$pdo->rollBack();response(['message'=>'Complete the entrepreneur bank account before recording this payout. Bank, branch, account holder and account number are required.'],422);}
