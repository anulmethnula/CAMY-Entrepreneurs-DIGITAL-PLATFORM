...
function storage_write(PDO $pdo,array $state): void {
    ...
    $orderWrite=$pdo->prepare('INSERT INTO shop_orders(id,group_id,entrepreneur_member_id,total,camy_amount,commission_amount,payment_method,payment_status,payment_collected_at,commission_status,commission_due_at,commission_paid_at,commission_reference,status,record_json) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE total=VALUES(total),camy_amount=VALUES(camy_amount),commission_amount=VALUES(commission_amount),payment_method=VALUES(payment_method),payment_status=VALUES(payment_status),payment_collected_at=VALUES(payment_collected_at),commission_status=VALUES(commission_status),commission_due_at=VALUES(commission_due_at),commission_paid_at=VALUES(commission_paid_at),commission_reference=VALUES(commission_reference),status=VALUES(status),record_json=VALUES(record_json)');
    $orderItem=$pdo->prepare('INSERT INTO shop_order_items(order_id,product_code,quantity,sell_price,camy_unit_price,commission_amount) VALUES(?,?,?,?,?,?)');
    foreach($state['orders'] ?? [] as $order){
        $groupId=$order['groupId'] ?? $order['id'];$groupWrite->execute([$groupId,$order['customer'] ?? '',$order['phone'] ?? '',$order['district'] ?? '',$order['address'] ?? '']);
        $orderWrite->execute([$order['id'],$groupId,$order['entrepreneurId'],$order['amount'],$order['camyAmount'] ?? $order['amount'],$order['commissionAmount'] ?? 0,$order['paymentMethod'] ?? 'bank_transfer',$order['paymentStatus'] ?? 'Awaiting payment',isset($order['paymentCollectedAt'])?date('Y-m-d H:i:s',strtotime($order['paymentCollectedAt'])):null,$order['commissionStatus'] ?? 'Not eligible',isset($order['commissionDueAt'])?date('Y-m-d H:i:s',strtotime($order['commissionDueAt'])):null,isset($order['commissionPaidAt'])?date('Y-m-d H:i:s',strtotime($order['commissionPaidAt'])):null,$order['commissionReference'] ?? null,$order['status'],storage_json($order)]);
        $pdo->prepare('DELETE FROM shop_order_items WHERE order_id=?')->execute([$order['id']]);
        foreach($order['items'] ?? [] as $item)$orderItem->execute([$order['id'],$products[(string)($item['id'] ?? $item['productId'])] ?? (string)($item['id'] ?? $item['productId']),$item['qty'],$item['price'],$item['basePrice'] ?? $item['price'],round(((float)$item['price']-(float)($item['basePrice'] ?? $item['price']))*(int)$item['qty'],2)]);
    }
