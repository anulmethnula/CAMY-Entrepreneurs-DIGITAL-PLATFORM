...
export function ShopHome({ person, inventory, products, requests, orders, setPage }) {
  const mine=inventory.filter(item=>item.entrepreneurId===person.id);const myOrders=orders.filter(order=>String(order.entrepreneurId)===String(person.id));const delivered=myOrders.filter(order=>order.status==='Delivered');const sales=delivered.reduce((sum,order)=>sum+Number(order.amount||0),0);const payable=myOrders.filter(order=>order.commissionStatus==='Payable').reduce((sum,order)=>sum+Math.max(0,Number(order.commissionAmount||0)),0)
  return <div className="content-page market-page"><div className="market-heading"><div><small>YOUR PRIVATE CAMY SHOP</small><h1>Welcome back, {person.name?.split(' ')[0]||'Entrepreneur'}.</h1><p>You find customers and set your retail prices. CAMY holds stock, collects customer money and fulfils every order.</p></div><a href={`/shops?shop=${encodeURIComponent(person.id)}`} target="_blank" rel="noreferrer"><Store size={17}/> Open my customer shop</a></div><ReturnAttention orders={myOrders} onOpen={()=>setPage('orders')}/><div className="shop-home-grid"><button onClick={()=>setPage('inventory')}><PackageOpen/><strong>My shop prices</strong><small>Set prices and choose visible CAMY products</small><ArrowRight/></button><button onClick={()=>setPage('orders')}><Check/><strong>Customer orders</strong><small>{myOrders.length} orders · {delivered.length} delivered</small><ArrowRight/></button><a href={`/shops?shop=${encodeURIComponent(person.id)}`} target="_blank" rel="noreferrer"><Store/><strong>Share my private link</strong><small>Customers see only your storefront</small><ArrowRight/></a></div><div className="shop-home-stats"><article><small>VERIFIED SALES</small><strong>{money(sales)}</strong><p>Only successfully delivered orders count</p></article><article><small>COMMISSION PAYABLE</small><strong>{money(payable)}</strong><p>CAMY settles eligible margin within 7 days</p></article><article><small>CAMY PRODUCTS LISTED</small><strong>{mine.filter(item=>item.visible!==false&&item.qty>0).length}</strong><p>Availability follows CAMY warehouse stock</p></article></div><section className="market-history"><h2>My shop products</h2>{mine.length?mine.map(item=>{const product=products.find(entry=>String(entry.id)===String(item.productId));return <article key={`${item.entrepreneurId}-${item.productId}`}><div><strong>{product?.name||'CAMY product'}</strong><small>{product?.code||''} · CAMY base {money(product?.price)} · My price {money(item.price)}</small></div><b>{item.visible!==false&&item.qty>0?'Live':'Hidden / unavailable'}</b></article>}):<p>Your CAMY catalogue will appear when products are activated.</p>}</section></div>
}
...
export function MyInventoryPage({ person, inventory, products, orders, savePrice, notify }) {
  ...
  const [details, setDetails] = useState(null)
  const mine = inventory.filter(item => item.entrepreneurId === person.id)
  ...
  return <div className="content-page market-page">
    <div className="market-heading"><div><small>MY PRIVATE SHOP</small><h1>Products and selling prices</h1><p>You do not hold physical stock. Choose your retail price for each CAMY product; CAMY’s base amount remains protected.</p></div></div>
    <ShopContactEditor initial={person.phone} notify={notify}/><BankEditor initial={person.bankDetails} notify={notify}/><div className="market-products">{mine.length ? mine.map(item => {
      ...
      return <article className="market-product" key={`${item.entrepreneurId}-${item.productId}`}>
        ...
        <div><small>{product.category} · {product.code}</small><h3>{product.name}</h3><p>CAMY base amount: {money(product.price)} · CAMY warehouse: {item.qty>0?'Available':'Out of stock'}</p>
          <button className="product-details-link" onClick={() => setDetails(product)}>View original CAMY details</button>
          <label className="shop-price">My customer selling price<input type="number" min="1" step="0.01" value={price} onChange={event => setDrafts(old => ({...old, [item.productId]: event.target.value}))}/><small>{Number(price)>=Number(product.price)?`Your commission: ${money(Number(price)-Number(product.price))}`:`You must contribute ${money(Number(product.price)-Number(price))} so CAMY still receives its base amount.`}</small></label>
    ...
    }) : <div className="market-empty"><PackageOpen/><h2>No CAMY products are active yet</h2><p>Products appear here when CAMY activates the catalogue.</p></div>}</div>
...
export function AdminSupplyPage({ requests, products, entrepreneurs, review, inventory, catalogueLive, activate, bank }) {
  ...
  return <div className="content-page market-page stock-admin-page">
    ...
    <details className="supply-bank-settings"><summary>CAMY payment bank account</summary><BankEditor initial={bank} purpose="customer"/></details>
