...
export function BankEditor({ initial, notify = () => {}, purpose = 'commission' }) {
  ...
  return <section className="workflow-panel"><h2>{purpose==='customer'?'CAMY customer payment account':'Commission settlement account'}</h2><p>{purpose==='customer'?'Customers use this CAMY account for approved bank-transfer orders. Entrepreneurs never receive customer payments directly.':'CAMY pays approved entrepreneur commission to this account. Customers never pay the entrepreneur directly.'}</p><div className="bank-editor">{[['bank', 'Bank'], ['branch', 'Branch'], ['holder', 'Account holder'], ['account', 'Account number']].map(([key, label]) => <label key={key}>{label}<input maxLength={120} value={bank[key] || ''} onChange={event => setBank({ ...bank, [key]: event.target.value })}/></label>)}</div><button className="market-primary" disabled={busy} onClick={save}>Save {purpose==='customer'?'CAMY payment':'settlement'} account</button>{message && <p role="status">{message}</p>}</section>
}
...
export function ReceiptUpload({ order, type = 'requests', token, onDone = () => {} }) {
  ...
  return <div className="workflow-panel"><BankDetails bank={order.bankDetails}/><p><strong>Approved amount: {money(order.total ?? order.amount)}</strong></p>{order.paymentNote && <p className="market-error">Receipt needs correction: {order.paymentNote}</p>}<p>After paying CAMY, upload the receipt below. CAMY Operations verifies payment before dispatch.</p><label>Payment reference<input maxLength={120} value={reference} onChange={event => setReference(event.target.value)}/></label><label>Bank receipt (up to 5 MB)<input type="file" accept="image/png,image/jpeg,image/webp,application/pdf" onChange={event => setFile(event.target.files?.[0] || null)}/></label><button className="market-primary" disabled={busy || !file || !reference.trim()} onClick={upload}>{busy ? 'Uploading…' : 'Submit receipt to CAMY'}</button>{error && <p className="market-error">{error}</p>}</div>
}
...
export function OrderReview({ order, onDone, admin = false }) {
  const [busy, setBusy] = useState(false)
  const [error, setError] = useState('')
  ...
  useEffect(()=>{if(admin)return;let active=true;api('/auth/me').then(result=>{if(active)setCanManage(['admin','manager'].includes(result.user?.role))}).catch(()=>{});return()=>{active=false}},[admin])
  const direct = order.orderOrigin === 'entrepreneur_portal' || order.submittedByEntrepreneur === true
  const actions = !canManage ? [] : { Pending: direct ? [['Processing','Confirm client order'],['Rejected','Reject order']] : order.paymentMethod === 'cash_on_delivery' ? [['Processing','Approve COD order'],['Rejected','Reject order']] : [['Awaiting payment','Approve bank-transfer order'],['Rejected','Reject order']], 'Awaiting payment': [['Rejected','Cancel unpaid order']], 'Payment review': [['Processing','Verify CAMY payment'],['Awaiting payment','Request corrected receipt']], Processing: [['Dispatched','Dispatch from CAMY']], Dispatched: [['Delivered','Mark delivered'],['Returned','Mark returned']], Delivered: [['Returned','Mark returned']] }[order.status] || []
  const act = async status => { if (busy) return; if (status === 'Dispatched' && !trackingNumber.trim()) return setError('Add the courier tracking number before dispatching.'); setBusy(true); setError(''); try { const result = await call(`orders/${order.id}/status`, { status, reason, trackingNumber: trackingNumber.trim(), courier: courier.trim() }); onDone(status, result.orders?.find(item => item.id === order.id)) } catch (error) { setError(error.message) } finally { setBusy(false) } }
  const settle = async action => { if(!reference.trim()||busy)return; setBusy(true);setError('');try{const result=await call(`orders/${order.id}/${action}`,{reference:reference.trim()});onDone(result.order?.status,result.order)}catch(failure){setError(failure.message)}finally{setBusy(false)} }
  return <section className="workflow-panel"><h3>CAMY order fulfilment</h3><p>Current stage: <strong>{order.status}</strong> · Payment: <strong>{order.paymentMethod === 'cash_on_delivery' ? 'Cash on delivery' : 'Bank transfer'}</strong> ({order.paymentStatus || 'Pending'})</p><div className="order-economics"><span>Customer total <b>{money(order.amount)}</b></span><span>CAMY base amount <b>{money(order.camyAmount ?? order.amount)}</b></span><span>{Number(order.commissionAmount||0)>=0?'Entrepreneur commission':'Entrepreneur contribution'} <b>{money(Math.abs(Number(order.commissionAmount||0)))}</b></span><span>Commission status <b>{order.commissionStatus || 'Not eligible'}</b></span></div>{order.commissionDueAt && <p>Commission settlement deadline: <strong>{new Date(order.commissionDueAt).toLocaleDateString()}</strong> (within 7 days of CAMY collecting payment).</p>}{order.receipt && <p><ReceiptPreview href={order.receipt} name="View bank receipt"/> · Reference: {order.reference}</p>}{order.status === 'Payment review' && canManage && <><p>Check the receipt against the CAMY bank account and exact order total before confirming payment.</p><label>Reason if receipt needs correction<input value={reason} onChange={event => setReason(event.target.value)}/></label></>}{order.status === 'Processing' && canManage && <div className="dispatch-fields"><h3>Delivery tracking</h3><p>Enter the CAMY parcel tracking number so the entrepreneur can follow the client delivery.</p><label>Courier / delivery company (optional)<input maxLength={120} value={courier} disabled={busy} onChange={event => setCourier(event.target.value)} placeholder="Courier company"/></label><label>Tracking number (required)<input required maxLength={120} value={trackingNumber} disabled={busy} onChange={event => setTrackingNumber(event.target.value)} placeholder="Parcel tracking number"/></label></div>}{order.trackingNumber && <p><b>Courier:</b> {order.courier || 'Not specified'}<br/><b>Tracking number:</b> <span className="tracking-number">{order.trackingNumber}</span></p>}<div className="workflow-actions">{actions.filter(([status]) => status !== 'Returned' || !order.return).map(([status,label]) => <button className="market-primary" key={status} disabled={busy || (status === 'Dispatched' && !trackingNumber.trim())} onClick={() => act(status)}>{label}</button>)}</div>{canManage && order.paymentMethod==='cash_on_delivery' && order.status==='Delivered' && order.paymentStatus!=='Collected' && <div className="commission-action"><label>Courier remittance / collection reference<input value={reference} onChange={event=>setReference(event.target.value)}/></label><button className="market-primary" disabled={busy||!reference.trim()} onClick={()=>settle('collect-cod')}>Confirm COD money collected by CAMY</button></div>}{canManage && order.commissionStatus==='Payable' && <div className="commission-action"><label>Entrepreneur payout bank reference<input value={reference} onChange={event=>setReference(event.target.value)}/></label><button className="market-primary" disabled={busy||!reference.trim()} onClick={()=>settle('pay-commission')}>Record commission paid</button></div>}{!canManage && <p>CAMY Operations controls payment verification, dispatch, delivery and commission settlement. You can monitor this order here.</p>}{error && <p className="market-error">{error}</p>}{!direct&&<DeliveryConfirmation order={order}/ >}{canManage&&<OrderReturns order={order} seller onDone={value => onDone(value.status,value)}/>}</section>
}
...
export function CustomerTracker({ tracking, canReview = false, reviews = [], onReview, onOrderUpdate }) {
  ...
  return <details className="shop-order-card" open={expanded} onToggle={event => setExpanded(event.currentTarget.open)}><summary className="shop-order-summary"><div className="shop-order-identity"><strong><Store size={15}/> {order?.entrepreneur || tracking.shop || 'Shop order'}</strong><small>{tracking.id} {order?.date ? `· ${order.date}` : ''}</small></div><span className={`shop-order-status ${status === 'Awaiting payment' ? 'needs-payment' : ''}`}>{order?.return && order.return.status !== 'Rejected' ? `Return ${order.return.status}${order.return.refundStatus === 'Refunded' ? ' ? Refunded' : ''}` : status === 'Processing' ? 'Payment verified' : status === 'Awaiting payment' ? 'Ready to pay' : status}</span><strong>{money(order?.amount ?? tracking.amount)}</strong><ChevronRight size={20}/></summary><div className="shop-order-body">
    {error && <p className="market-error" role="alert">{error}</p>}{order && <>
      ...
      {status === 'Pending' && <p>CAMY is reviewing stock and your selected payment method. Please wait before paying.</p>}
      {status === 'Awaiting payment' && <><p><b>Pay {money(order.amount)} to CAMY only.</b> Use this order number as your reference. Never transfer customer money to the entrepreneur.</p><ReceiptUpload order={order} type="orders" token={tracking.token} onDone={receiveOrder}/></>}
      {status === 'Payment review' && <p>Your receipt is with CAMY for verification. You do not need to pay again.</p>}
      {status === 'Processing' && <p>{order.paymentMethod==='cash_on_delivery'?'CAMY approved this cash-on-delivery order and is preparing it for dispatch.':'CAMY verified your payment and is preparing the items for dispatch.'}</p>}
      ...
      {status === 'Delivered' && <p>Your order has been delivered. {order.paymentMethod==='cash_on_delivery'?'Your COD payment is collected for CAMY. ':''}Thank you for supporting {order.entrepreneur || 'your local entrepreneur'}.</p>}
