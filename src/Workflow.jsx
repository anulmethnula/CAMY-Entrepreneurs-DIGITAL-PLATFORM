...
export function OrderReview({ order, onDone }) {
  ...
  const actions = { Pending: [['Awaiting payment','Approve order'],['Rejected','Reject request']], 'Awaiting payment': [['Rejected','Cancel unpaid order']], 'Payment review': [['Processing','Verify payment'],['Awaiting payment','Request corrected receipt']], Processing: [['Dispatched','Dispatch order']], Dispatched: [['Delivered','Mark delivered'],['Returned','Mark returned']], Delivered: [...(!order.deliveryConfirmations?.seller ? [['Delivered','Confirm delivery as seller']] : []),['Returned','Mark returned']] }[order.status] || []
  ...
  return <section className="workflow-panel"><h3>Order approval and payment</h3><p>Current stage: <strong>{order.status}</strong></p>{order.receipt && <p><ReceiptPreview href={order.receipt} name="View bank receipt"/> · Reference: {order.reference}</p>}{order.status === 'Payment review' && <><p>Check the receipt against your bank account and the order total before confirming payment.</p><label>Reason if receipt needs correction<input value={reason} onChange={event => setReason(event.target.value)}/></label></>}{order.status === 'Processing' && <div className="dispatch-fields"><h3>Delivery tracking</h3><p>Enter the parcel tracking number so your customer can follow the delivery.</p><label>Courier / delivery company (optional)<input maxLength={120} value={courier} disabled={busy} onChange={event => setCourier(event.target.value)} placeholder="Courier company"/></label><label>Tracking number (required)<input required maxLength={120} value={trackingNumber} disabled={busy} onChange={event => setTrackingNumber(event.target.value)} placeholder="Parcel tracking number"/></label></div>}{order.trackingNumber && <p><b>Courier:</b> {order.courier || 'Not specified'}<br/><b>Tracking number:</b> <span className="tracking-number">{order.trackingNumber}</span></p>}<div className="workflow-actions">{actions.filter(([status]) => status !== 'Returned' || !order.return).map(([status,label]) => <button className="market-primary" key={status} disabled={busy || (status === 'Dispatched' && !trackingNumber.trim())} onClick={() => act(status)}>{label}</button>)}</div>{error && <p className="market-error">{error}</p>}<DeliveryConfirmation order={order}/><OrderReturns order={order} seller onDone={value => onDone(value.status,value)}/></section>
}
