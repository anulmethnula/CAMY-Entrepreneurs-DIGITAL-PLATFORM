...
import { ArrowRight, BadgeDollarSign, CheckCircle2, Clock3, PackageCheck, PackageOpen, RefreshCw, ShoppingCart, Truck, X } from 'lucide-react'
import { api } from './api'
...
function statusHelp(order) {
  if (order.status === 'Processing') return 'CAMY received this order and is preparing the parcel.'
  ...
  if (order.status === 'Delivered') return 'Delivery completed. This sale now counts toward your verified sales and credit tier.'
...
function CreditSensor({ person, tiers = [], orders = [] }) {
  ...
  const sales = delivered.reduce((sum, order) => sum + Number(order.amount || 0), 0)
  ...
  return <section className="shop-home-stats">
    <article><small>VERIFIED SALES</small><strong>{money(sales)}</strong><p>Delivered CAMY orders only</p></article>
    <article><small>CREDIT LIMIT</small><strong>{money(credit)}</strong><p>{credit > 0 ? 'Current eligible credit' : 'Build verified sales to unlock credit'}</p></article>
    <article><small>NEXT CREDIT SENSOR</small><strong>{next ? money(remaining) : 'Top tier'}</strong><p>{next ? `More verified sales to unlock ${money(next.credit)}` : 'Highest configured tier reached'}</p></article>
  </section>
}
...
export function DropshipOrderPage({ products = [], person, notify, catalogueLive }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      notify?.(`${result.order.id} sent to CAMY. CAMY will handle packing and delivery.`)
      setCart([])
  ...
  return <div className="content-page market-page stock-buy-page">
    ...
    <section className="market-history stock-request-history">
      ...
      {orders.length ? [...orders].sort((a,b)=>String(b.createdAt||b.date).localeCompare(String(a.createdAt||a.date))).map(order => <article className="stock-tracker-entry" key={order.id}>
        <div><strong>{order.id} · {order.customer}</strong><small>{(order.items || []).map(item => `${item.name} × ${item.qty}`).join(', ')} · {money(order.amount)}</small><p>{statusHelp(order)}</p></div>
