...
export function AdminOrderWorkspace({ children }) {
  const [tab,setTab]=useState('orders')
  return <><nav className="admin-order-workspace-tabs" aria-label="Orders and feedback"><button className={tab==='orders'?'active':''} onClick={()=>setTab('orders')}>Customer orders</button><button className={tab==='feedback'?'active':''} onClick={()=>setTab('feedback')}>Ratings & feedback</button></nav>{tab==='orders'?children:<AdminCustomerReviews/>}</>
}
