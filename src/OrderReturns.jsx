...
export function OrderReturns({ order, seller = false, readOnly = false, onDone }) {
  ...
  if (!record && ((seller && !dropship) || readOnly || order.status !== 'Delivered') && order.status !== 'Returned') return null
  ...
  return <section className="order-return-panel"><h3>Returns &amp; refunds</h3>
    ...
    {!record && order.status === 'Delivered' && !readOnly && <>{dropship&&seller?<><p>Record a return only when the client has contacted CAMY/entrepreneur after a completed COD delivery. This starts the tracked refund workflow.</p><label>Client return reason<textarea rows={3} minLength={8} maxLength={1000} value={reason} onChange={event => setReason(event.target.value)} /></label><button className="market-primary" disabled={busy || reason.trim().length < 8} onClick={() => act('request')}>Record client return request</button></>:<><p>Request a return of this entire shop order, including all its items. Wait for the shop’s approval and return address before sending anything.</p><label>Why would you like to return this order?<textarea rows={3} minLength={8} maxLength={1000} value={reason} onChange={event => setReason(event.target.value)} /></label><button className="market-primary" disabled={busy || reason.trim().length < 8} onClick={() => act('request')}>Request return</button></>}</>}
    ...
    {record && <><p><strong>Return: {record.status}</strong></p><p><b>Customer’s reason:</b> {record.reason}</p>
      {record.status === 'Requested' && <p>{dropship?'CAMY is reviewing the recorded client return request.':'The shop is reviewing this request. Keep the items until the return is approved.'}</p>}
      ...
      {record.status === 'Received' && <p><b>Client refund: {record.refundStatus}</b> · Rs. {Number(record.refundAmount).toLocaleString('en-LK')}<br />{record.refundReference ? `Refund reference: ${record.refundReference}` : dropship?'CAMY must complete and record the client refund.':'The shop must arrange the refund by bank transfer outside this platform.'}</p>}
      ...
      {seller && record.status === 'Received' && record.refundStatus === 'Pending' && <><label>{dropship?'Client refund reference':'Bank refund reference'}<input maxLength={120} value={reference} onChange={event => setReference(event.target.value)} /></label><button className="market-primary" disabled={busy || !reference.trim()} onClick={() => act('refund')}>{dropship?'Record client refund completed':'Record completed bank refund'}</button></>}
    </>}
